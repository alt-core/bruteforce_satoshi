...
def main(problem_id, pattern, n_rescan=0, charset='%lower', flag_list=False):
    ...
    if n_rescan == 0:
        ...
        for indices in itertools.combinations(rescan_indices, n_rescan):
            ...
            candidates.update({''.join(prod) for prod in itertools.product(*tmp_parsed)})
    ...
    if flag_list:
        ...
        print(f'evaluate {len(candidates)} candidates')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--list', dest='list', action='store_true', default=False, help='show candidates')
    parser.add_argument('-s', '--rescan', dest='rescan', default=0, help='replace N single chars to ? by rotation. default: 0')
    parser.add_argument('-d', '--dic', '--dictionary', dest='dictionary', default=None, help='you can refer the dictionary as %%dic')
    parser.add_argument('-c', '--charset', dest='charset', default='%lower', help='default: %%lower')
    args = parser.parse_args()
    if args.dictionary is not None:
        with open(args.dictionary, 'r') as fp:
            dictionary = [line.strip() for line in fp]
            charsets['dic'] = dictionary
    main(args.problem_id, args.pattern, n_rescan=args.rescan, charset=args.charset, flag_list=args.list)
